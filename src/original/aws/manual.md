...
3. 講師から伝えられたユーザー名及びパスワードを入力して [サインイン] ボタンを押下してください
    ...
    ::: tip 
    AWSのマネージメントコンソールは、ユーザーの要望を取り入れて日々進化しています。画面のレイアウトや表記が本手順書と若干異なる場合がありますが、基本的な操作の流れは同じです
    :::
...
#### 手順
1. 画面上部のナビゲーションバーにある検索ボックスに**EC2**と入力します（小文字でも可）。表示された一覧から**EC2**をクリックしてください
    > EC2という文字をクリックしてください。他の箇所はクリックしても反応しません
4. EC2のダッシュボードが表示されます
5. 左側のメニューにて、**ネットワーク＆セキュリティ**の中にある**セキュリティグループ**をクリックしてください
...
13. 続けて、データベース用のセキュリティグループも作成します。左側メニューにて**ネットワーク＆セキュリティ**の中にある**セキュリティグループ**をクリックしてください。
...
#### 手順 
1. 画面上部のナビゲーションバーにある検索ボックスに**RDS**と入力します（小文字でも可）。表示された一覧から Aurora and RDS をクリックしてください
1. RDSのダッシュボードが表示されます。左側メニューにて**データベース**をクリックしてください。データベース一覧画面に遷移します
3. 右側の [データベースの作成] ボタンを押下してください

    :::warning
    画面上部に「エクスプレス設定」と「フル設定」の切り替えが表示されることがあります。必ず**フル設定**を選択してください。エクスプレス設定では細かい設定ができないため、この演習では使用しません
    :::

2. **データベース作成方法を選択** は、「フル設定」のままにしてください
4. エンジンのオプションにて、**MariaDB**のタイルをクリックしてください
5. エンジンバージョンは、**10.11.16**を選択します。もし10.11.16がない場合は、10.11.x の x が最大のものを選択してください
6. テンプレートでは、**開発/テスト**をクリックしてください
7. **設定**にて、以下を設定します

    :::warning
    もし、 DBインスタンス識別子以外の項目が表示されていない場合は、「認証情報の設定」をクリックしてください。設定項目が表示されます
    :::

    |項目|設定値
    |:--|:--
    |DBインスタンス識別子|sawakura-lab-db
    |マスターユーザー名|admin
    |認証情報管理|セルフマネージド
    |マスターパスワード|password
    |マスターパスワードを確認|password
...
10. ストレージに関する設定では、**ストレージ割り当て**の値を、20GiBに変更してください
11. **ストレージの自動スケーリングを有効にする** のチェックが入っている場合は、チェックを外してください
    :::warning
    ストレージの割り当ては、必ず 20GiB に変更してください（元の値は、200GiBです）。また、ストレージの自動スケーリングは必ず無効にしてください
    :::

12. 可用性と耐久性では、**スタンバイインスタンスを作成しないでください**にチェックが入っていることを確認して次に進んでください
13. **接続**では、以下を設定します
    ...
    |既存のVPCセキュリティグループ（ファイアーウォール）|既存のVPCセキュリティグループ のボックスをクリックし、表示されたものから db-sg を選択。続けて、「Default」の右にあるXをクリックして、Defaultを削除
...
14. 追加設定をクリックして開き、データベースポートが 3306 であることを確認してください
15. モニタリングの項目で、**拡張モニタリングの有効化** のチェックを外してください
16. 追加設定の三角をクリックして、以下を設定してください
    ...
    |暗号化|チェックを外してください
    |削除保護の有効化|**チェックをしないでください**<br>チェックされてない場合はそのままで結構です

17. 一番下まで進み、[データベースの作成] ボタンを押下してください。しばらく待つとデータベースの一覧画面が表示されます。sawakura-lab-dbの推奨アドオン、、、というウィンドウが出てきたら、右下の[閉じる]ボタンを押して下さい

    ::: warning
    ご利用のブラウザによっては、パスワードの侵害という警告が出ることがありますが、無視して警告を閉じてください。また、ブラウザでのパスワードの保存は不要です。
    :::

18. データベースインスタンスの作成が完了するまで少し時間がかかります。完了を待たずに次に進んでください
...
#### 手順 
### 4-1. EC2インスタンス作成
1. 画面上部のナビゲーションバーにある検索ボックスに**EC2**と入力します（小文字でも可）。表示された一覧からEC2をクリックしてください。EC2のダッシュボードが表示されます
...
1. クイックスタートタブの下の、Amazon Linuxが選択されていることを確認してください。**Amazon Linux 2023 AMI** と表示されていれば問題ありません
1. **アーキテクチャ**が、 **64ビット (x86)** になっていることを確認してください
...
7.  次のキーペアは今回は利用しません。ドロップダウンボックスから、 **キーペアなしで続行** を選択してください
    :::tip
    キーペアは、インスタンスにSSHログインするための認証鍵です。今回はSSHではない方法でインスタンスにログインするので不要です。「キーペアなしで続行」が見つからない場合は、「キーペアを使用しない」等の類似の選択肢を選んでください
    :::
...
23. これで設定は完了です。[インスタンスの起動] ボタンを押下してください。画面が遷移し、情報が表示されます。[すべてのインスタンスの表示] ボタンを押下してください
...
#### 手順
1. 画面上部のナビゲーションバーにある検索ボックスに**RDS**と入力します（小文字でも可）。表示された一覧から**Aurora and RDS**をクリックします
...
1. 画面上部のナビゲーションバーにある検索ボックスに**EC2**と入力します（小文字でも可）。表示されたメニューの**EC2**をクリックします
1. 左側メニューにて**インスタンス**をクリックしてください。インスタンス一覧に、先程起動したEC2インスタンスが表示されます
1. インスタンスの左側のチェックボックスを選択し、画面上部の [接続] ボタンをクリックしてください
...
3. 接続ボタンを押下すると、しばらくしてインスタンスのシェルに接続され、Linuxコマンドが実行可能な状態になります。**sh-5.2$** もしくは **$** のようなプロンプトが表示されるまで、少しお待ち下さい 
4. Webアプリケーションの配置されているLinuxユーザーに切り替えます。次のコマンドを実行してください
    > プロンプト（$）の後にコピー＆ペーストしてからEnterキー、で実行できます
...
7.  問題なく起動すると、以下のようなメッセージが表示され、Webブラウザからのアクセスを受け付けられる状態になります（表示内容は環境により異なります）
    ...
    * Running on http://10.0.1.xx:80/ (Press CTRL+C to quit)
    ...
    :::warning
    赤い文字で WARNING. This is a development server... と表示されますが、エラーではないので次に進んでください。これは、今起動しているのは開発環境向けのテストサーバーなので商用環境で使わないように、という警告メッセージです
    :::
