...
5. バージョンは、**10.11.8**を選択します。もし10.11.8がない場合は、10.11.x の x が最大のものを選択してください
...
10. ストレージに関する設定では、**ストレージ割り当て**の値を、20GiBに変更してください
    :::warning
    ストレージの割り当ては、必ず 20GiB に変更してください（元の値は、200GiBです）
    :::
...
14. 一番下まで進み、[データベースの作成] ボタンを押下してください。しばらく待つとデータベースの一覧画面が表示されます。sawakura-lab-dbの推奨アドオン、、、というウィンドウが出てきたら、右下の[閉じる]ボタンを押して下さい
...
3. 接続ボタンを押下すると、しばらくしてインスタンスのシェルに接続され、Linuxコマンドが実行可能な状態になります。**sh-5.2$** と表示されるまで、少しお待ち下さい 
